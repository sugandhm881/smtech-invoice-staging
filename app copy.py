...
def generate_upi_qr_base64(upi_id, upi_name, amount):
    """Generate UPI QR code with amount pre-filled, return base64 PNG"""
    try:
...
def PDF_Generator(invoice_data, is_credit_note=False, is_debit_note=False):
    ...
    # CHANGE 4: Determine invoice_type (goods/service) from profile
    # Master sets invoice_type in profile; default is 'goods'
    ...
    # CHANGE 4: Only show Ship To for goods invoices, not services
    if not is_service:
        y_start = pdf.get_y()
        pdf.set_font("Calibri", "B", 12)
    ...
    # CHANGE 4: For service invoices, rename "Rate (Incl. Tax)" to "Rate (Excl. Tax)"
    rate_col_label = "Rate (Excl.)" if is_service else "Rate (Incl.)"
    
    particulars_w, hsn_w, qty_w, rate_w, disc_w, tax_percent_w, taxable_amt_w, tax_amt_w, total_w = 46, 15, 12, 18, 12, 12, 22, 18, 25
    ...
    pdf.cell(rate_w, 8, rate_col_label, 1, 0, 'R', True)
    pdf.cell(disc_w, 8, "Disc%", 1, 0, 'R', True) 
    ...
    for i in range(len(particulars)):
        ...
        try:
            disc_p = float(discounts[i])
            disc_str = f"{disc_p:.1f}%" if disc_p > 0 else "-"
        except: disc_str = "-"
        ...
        pdf.cell(disc_w, row_h, disc_str, 1, 0, 'R') 
    ...
    def add_total(label, val):
        pdf.cell(137, 7, label, 1, 0, 'R', True) 
        pdf.cell(43, 7, f"{abs(val):.2f}", 1, 1, 'R', True) 
    
    total_discount = invoice_data.get('total_discount', 0)
    if total_discount > 0:
        add_total("Total Discount", total_discount)
    ...
    # CHANGE 2: Add UPI QR code if UPI ID is configured in profile
    upi_id_profile = profile.get('upi_id', '').strip()
    upi_name_profile = profile.get('company_name', UPI_NAME)
    grand_total_val = invoice_data.get('grand_total', 0)
    
    if upi_id_profile and grand_total_val > 0 and not is_credit_note and not is_debit_note:
        try:
            qr_b64 = generate_upi_qr_base64(upi_id_profile, upi_name_profile, abs(grand_total_val))
            if qr_b64:
                ...
                pdf.image(qr_tmp_path, x=margin, y=pdf.get_y(), w=28, h=28)
                pdf.set_font("Calibri", "", 8)
                pdf.set_xy(margin + 30, pdf.get_y())
                pdf.multi_cell(60, 4, f"UPI ID: {upi_id_profile}\nAmount: Rs. {abs(grand_total_val):.2f}\n(Scan to pay)")
                pdf.ln(20)
                os.unlink(qr_tmp_path)
        except Exception as e:
            logging.error(f"UPI QR in PDF error: {e}")
...
def generate_excel_bytes(user_id):
    ...
    ws.append([
        ...
        "Item Name", "HSN", "Qty", "Rate (Incl Tax)", "Disc %",
    ...
    for inv in invoices:
        ...
        for i in range(len(part_list)):
            ws.append([
                ...
                float(disc_list[i]) if i < len(disc_list) else 0,
...
def login():
    ...
    if request.method=="POST":
        ...
        if valid_creds:
            ...
            # CHANGE 8: Send OTP to the user's own saved profile email
            target_email = None
            try:
                if is_master:
                    master_profile_doc = db.collection('config').document('seller_profile').get()
                    if master_profile_doc.exists:
                        target_email = master_profile_doc.to_dict().get('email')
                else:
                    # Sub-user: get their profile email first, fallback to app_users doc email
                    user_profile_doc = db.collection('users').document(username).collection('config').document('profile').get()
                    if user_profile_doc.exists:
                        target_email = user_profile_doc.to_dict().get('email')
                    if not target_email:
                        user_doc_data = db.collection('app_users').document(username).get()
                        if user_doc_data.exists:
                            target_email = user_doc_data.to_dict().get('email')
            except Exception as e:
                logging.error(f"Error fetching user email for OTP: {e}")

            # Final fallback to master email or system email
            if not target_email:
                try:
                    master_profile_doc = db.collection('config').document('seller_profile').get()
                    if master_profile_doc.exists:
...
def activation_page():
    if request.method == "POST":
        ...
        req_data = {
            ...
            "date_display": date.today().strftime('%d-%b-%Y')  # CHANGE 6: DD-MM-YYYY
        }
        try:
...
def user_profile():
    ...
    if request.method == 'POST':
        ...
        if 'new_username' in request.form:
            ...
            if new_u and new_p:
                db.collection('app_users').document(new_u).set({
                    "password": generate_password_hash(new_p), 
                    "is_active": False,
        ...
        # --- RENAME SUB-USER ---
        if 'action_rename_user' in request.form:
            if not current_user.is_master: 
                return "Unauthorized", 403
            
            old_username = request.form.get('target_user_id')
            new_username = request.form.get('new_sub_username', '').strip()
            
            if not new_username or old_username == MASTER_USERNAME or new_username == MASTER_USERNAME:
                flash("Invalid username operation.", "error")
                return redirect(url_for('user_profile', edit_user=old_username))
                
            # Check if new username already exists
            existing_check = db.collection('app_users').document(new_username).get()
            if existing_check.exists:
                flash(f"Username '{new_username}' is already taken!", "error")
                return redirect(url_for('user_profile', edit_user=old_username))
                
            # 1. Copy Authentication Doc
            old_user_doc = db.collection('app_users').document(old_username).get()
            if old_user_doc.exists:
                db.collection('app_users').document(new_username).set(old_user_doc.to_dict())
                db.collection('app_users').document(old_username).delete()
                
            # 2. Copy Profile Settings
            old_profile_doc = db.collection('users').document(old_username).collection('config').document('profile').get()
            if old_profile_doc.exists:
                db.collection('users').document(new_username).collection('config').document('profile').set(old_profile_doc.to_dict())
                db.collection('users').document(old_username).collection('config').document('profile').delete()
                
            # 3. Copy Counters
            old_counters_doc = db.collection('users').document(old_username).collection('config').document('counters').get()
            if old_counters_doc.exists:
                db.collection('users').document(new_username).collection('config').document('counters').set(old_counters_doc.to_dict())
                db.collection('users').document(old_username).collection('config').document('counters').delete()
                
            # 4. Migrate All Business Data
            collections_to_move = [
                'sales_invoices', 'sales_credit_notes', 'sales_debit_notes', 
                'purchase_orders', 'purchase_grns', 'purchase_bills', 'purchase_debit_notes', 
                'invoices', 'clients', 'particulars', 'payments'
            ]
            for coll in collections_to_move:
                docs = db.collection('users').document(old_username).collection(coll).stream()
                for d in docs:
                    db.collection('users').document(new_username).collection(coll).document(d.id).set(d.to_dict())
                    db.collection('users').document(old_username).collection(coll).document(d.id).delete()

            flash(f"User '{old_username}' successfully renamed to '{new_username}'.", "success")
            return redirect(url_for('user_profile', edit_user=new_username))
        ...
        data = {
            ...
            "upi_id": request.form.get('upi_id', '').strip(),  # CHANGE 2: Save UPI ID
        }

        # CHANGE 4: Save invoice_type (only master can set, only master sees it)
        if current_user.is_master:
            invoice_type_val = request.form.get('invoice_type', 'goods')
            data['invoice_type'] = invoice_type_val
...
def handle_invoice():
    try:
        ...
        # CHANGE 4: Get invoice_type from data (sent by frontend)
        ...
        if auto_generate:
            ...
            invoice_date_str = date.today().strftime('%d-%b-%Y')  # CHANGE 6: DD-MM-YYYY
        else:
            ...
            if manual_date:
                try:
                    ...
                    invoice_date_str = dt.strftime('%d-%b-%Y')  # CHANGE 6
                except:
                    invoice_date_str = date.today().strftime('%d-%b-%Y')
            else:
                invoice_date_str = date.today().strftime('%d-%b-%Y')
        
        # --- 7. CALCULATIONS (WITH GSTIN / STATE FALLBACK) ---
        profile = get_seller_profile_data()
        my_gstin = profile.get('gstin', '').strip()
        seller_state_name = profile.get('state', '').strip()
        my_state_code = my_gstin[:2] if len(my_gstin) >= 2 else None

        client_gstin = data.get('client_gstin', '').strip()
        client_state = data.get('client_state', '').strip()

        is_intra = True 
        
        if my_state_code and client_gstin:
            is_intra = client_gstin.startswith(my_state_code)
        elif seller_state_name and client_state:
            is_intra = seller_state_name.lower() == client_state.lower()
        elif client_state and not seller_state_name:
            is_intra = True 
        ...
        for i in range(len(amounts_inclusive)):
            inclusive_or_excl = float(amounts_inclusive[i])
            qty_val = float(qtys[i]) if i < len(qtys) else 0.0
            rate_val = float(rates[i]) if i < len(rates) else 0.0
            tax_rate = 0 if is_non_gst else (float(taxrates[i]) if i < len(taxrates) else 0)

            if is_service:
                # CHANGE 4: For service, amounts_inclusive is already excl tax
                taxable = round(inclusive_or_excl, 2)
                tax_amt = round(taxable * tax_rate / 100, 2)
                total_val = taxable + tax_amt
                line_gross = qty_val * rate_val
                if line_gross > taxable:
                    total_discount_amt += (line_gross - taxable)
            else:
                # Original: amounts are inclusive of tax
                line_gross = qty_val * rate_val
                if line_gross > inclusive_or_excl:
                    total_discount_amt += (line_gross - inclusive_or_excl)
                taxable = round(inclusive_or_excl / (1 + tax_rate/100), 2)
                tax_amt = round(inclusive_or_excl - taxable, 2)
                total_val = inclusive_or_excl
            
            line_taxable.append(taxable)
            line_tax.append(tax_amt)
            line_total.append(round(total_val, 2))
            
            if not is_non_gst:
                if is_intra:
                    cgst_amt = round(tax_amt / 2, 2)
                    total_cgst += cgst_amt
                    total_sgst += tax_amt - cgst_amt
                else:
                    total_igst += tax_amt
        ...
        invoice_data = {
            ...
            "doc_type": doc_type,
            "invoice_type": invoice_type,  # CHANGE 4
            ...
            "total_discount": round(total_discount_amt, 2),
            "sub_total": round(sum(line_taxable), 2),
            "igst": round(total_igst, 2),
            "cgst": round(total_cgst, 2),
            "sgst": round(total_sgst, 2),
            "grand_total": round(sum(line_total), 2),
            ...
            if not is_edit_mode:
                ...
                if direction != 0:
                    ...
                    for k in range(len(parts)):
                        ...
                        valid_items.append((iname, iqty, safe_id))
                        
                        if safe_id not in p_refs:
                            p_ref = db.collection('inventory_products').document(safe_id)
                            p_refs[safe_id] = p_ref
                            snap = p_ref.get(transaction=transaction) 
                            tracker[safe_id] = {
                                "exists": snap.exists,
                                "stock": float(snap.get('current_stock') or 0) if snap.exists else 0.0
                            }

            if not is_edit_mode and direction != 0:
                ts = datetime.now().isoformat()
                for iname, iqty, safe_id in valid_items:
                    p_ref = p_refs[safe_id]
                    cur_stock = tracker[safe_id]["stock"]
                    new_stock = cur_stock + (iqty * direction)
                    tracker[safe_id]["stock"] = new_stock
                    
                    if not tracker[safe_id]["exists"]:
                         transaction.set(p_ref, {"item_name": iname, "current_stock": new_stock, "last_updated": ts})
                         tracker[safe_id]["exists"] = True
                    else:
                    ...
                    transaction.set(l_ref, {
                        ...
                        "doc_type": f"{inv_data.get('doc_category')}_{inv_data.get('doc_type')}",
                        "item_name": iname,
...
def send_daily_report():
    ...
    if token != os.getenv('CRON_SECRET', 'change-this-cron-token'):
            ...
            if should_run:
                ...
                if excel_bytes:
                    ...
                    if seller_email:
                        ...
                        send_email_with_attachment(seller_email, subject, body, excel_bytes, f"Report_{date.today().strftime('%d-%b-%Y')}.xlsx")
...
def generate_credit_note(bill_no):
    try:
        ...
        cn_data['invoice_date'] = date.today().strftime('%d-%b-%Y')  # CHANGE 6
...
def download_excel_report():
    try:
        ...
        return send_file(excel_bytes, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', as_attachment=True, download_name=f'Sales_Report_{date.today().strftime("%d-%m-%Y")}.xlsx')
    except Exception as e:
...
def download_gstr1():
    try:
        ...
        for inv in sales_docs:
            ...
            # Apply Month/Year Filter (Matches "Mar 2026" format)
            if month_year:
                parts = inv_date.split('-')
                if len(parts) == 3:
                    inv_my = f"{parts[1]} {parts[2]}"
                    if inv_my != month_year:
                        continue
                else:
                    continue # Skip invalid dates if filter applied
...
def add_payment():
    try:
        ...
        payment_date = data.get('payment_date', date.today().strftime('%d-%b-%Y'))  # CHANGE 6
...
def outstanding_report():
    try:
        ...
        for inv in sales_inv:
            ...
            try:
                # CHANGE 6: Parse DD-MM-YYYY format
                parts = inv_date_str.split('-')
                if len(parts) == 3:
                    # Try DD-MM-YYYY first, then DD-Mon-YYYY for legacy
                    try:
...
def dashboard_data():
    try:
        ...
        for inv in invoices:
            ...
            try:
                parts = inv_date_str.split('-')
                # CHANGE 6: Handle both DD-MM-YYYY (new) and DD-Mon-YYYY (legacy)
                if len(parts) == 3:
                    try:
                        inv_date = date(int(parts[2]), int(parts[1]), int(parts[0]))
                    except ValueError:
                ...
                inv_month_label = inv_date.strftime('%b %y')
            ...
            if cat == 'sale' and not is_cn and dtype == 'invoice':
                ...
                if inv_month_num == this_month_num and inv_year == this_year: month_sales += amount

                if inv_date:
                    monthly_trend[inv_month_label] = monthly_trend.get(inv_month_label, 0) + amount
                ...
                if inv_month_num == this_month_num and inv_year == this_year: month_purchase += amount
...
def reset_password():
    ...
    if target and new_pass:
        db.collection('app_users').document(target).set({"password": generate_password_hash(new_pass)}, merge=True)
